   ...
   */
   ...
   public String decodeString(StringPart[] parts)
   {
      ...
      for(StringPart s: parts)
         result = result + masterString.substring(s.getStart(), s.getStart()+s.getLength());
      return result;
                                                     // 1
   }
   ...
   */
   ...
   public StringPart[] encodeString(String word)
   {
      ...
      while(!tempWord.isEmpty()){
         StringPart tempPart = findPart(tempWord);
         temp[count] = tempPart;
         tempWord = tempWord.substring(tempPart.getLength());
         count++;
      }

      StringPart[] result = new StringPart[count];
